...
class EpsilonGreedy(nn.Module):
    def __init__(self, config, act_space, greedy_behaviour):
        super(EpsilonGreedy, self).__init__()
        self.greedy_behaviour = greedy_behaviour
        self.random_behaviour = Random(config, act_space)
        self.epsilon = config.epsilon

    def actor(self, feat):
        # decide whether to explore or exploit
        if torch.rand(1) > self.epsilon:
            return self.greedy_behaviour.actor(feat)
        else:
            return self.random_behaviour.actor(feat)

    def train(self, start, context, data):
        # return self.greedy_behaviour.train(start, context, data)
